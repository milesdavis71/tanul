(function ($, w) {
	...
	$window.on("elementor/frontend/init", function () {
		var Particles = function ($scope) {
			//console.log($scope.data( 'element_type' ), $scope.hasClass( "ha-particle-yes" ));
			...
			var id = $scope.data("id"),
				...
				particles_style = $scope
					.find(".ha-particle-wrapper")
					.data("ha-partstyle");

			if (typeof particles_style != "undefined") {
				if (element_type == "column") {
					var $column = $scope.find(
				...
				if (element_type == "column") {
					if (
						$scope.find(".elementor-background-overlay").length == 0
					) {
			...
			if (particles_custom !== "" && particles_style == "custom") {
				...
				if ($scope.hasClass("ha-particle-adv-yes")) {
					...
					var particles_interactive =
						$selector.data("ha-interactive");
