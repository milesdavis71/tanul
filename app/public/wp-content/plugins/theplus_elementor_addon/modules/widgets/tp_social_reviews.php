...
class ThePlus_Social_Reviews extends Widget_Base {
	...
	protected function render() {
	   ...
	   //layout
		...
		$output .= '<div id="'.esc_attr($uid_socirw).'" class="'.esc_attr($uid_socirw).' tp-social-reviews '.esc_attr($data_class).'" '.$layout_attr.' '.$data_attr.' data-enable-isotope="1"  data-scroll-normal='.esc_attr($NormalScroll).'>';
    ...
    protected function Facebook_Reviews($RData){
	    ...
	    if($facebook_status == 200){
	        ...
	        $Error = (!empty($Fbdata['error']) ? $Fbdata['error'] : []);
	        $FbArr[] = array(
	            "Reviews_Index" => 0,
				"ErrorClass"    => "danger-error",
	            "Type" 			=> !empty($Error['type']) ? $Error['type'] : '',
	            "CreatedTime" 	=> '',
	            "Massage" 		=> !empty($Error['message']) ? $Error['message'] : 'Something Wrong',
	            "UserName" 		=> !empty($Fbdata['HTTP_CODE']) ? 'Error No : '.$Fbdata['HTTP_CODE'] : 400,
	            "UserImage" 	=> $Fb_Icon,
	            "Logo"          => $Fb_Icon,
	            "selectType"    => $ReviewsType,
	            "FilterCategory"=> $RCategory,
	...
	protected function Google_Reviews($RData){
	    ...
	    if($G_status == 200 && empty($Gdata['error_message'])){
	        ...
	        $GArr[] = array(
	            "Reviews_Index" => 1,
	            "ErrorClass"    => "danger-error",
	            "CreatedTime" 	=> !empty($Gdata['status']) ? $Gdata['status'] : '',
	            "Massage" 		=> !empty($Gdata['error_message']) ? $Gdata['error_message'] : '',
	            "UserName" 		=> !empty($Gdata['HTTP_CODE']) ? 'Error No : '.$Gdata['HTTP_CODE'] : '',
	            "UserImage" 	=> $GG_Icon,
	            "Logo"          => $GG_Icon,
	            "selectType"    => $ReviewsType,
	            "FilterCategory"=> $RCategory,
    ...
    protected function Beach_Reviews($attr){
	    ...
	    if($Beach_CODE == 200 && empty($Data['error_message'])){
	        ...
	        if($BType == "b-facebook"){
	            ...
	            $message = !empty($Error['message']) ? str_replace(". ","<br/>", $Error['message']) : 'Something wrong';
	        }
	        if($BType == "b-google"){
