...
if(isset($theplus_optionsget['check_elements']) && !empty($theplus_optionsget['check_elements']) && in_array("tp_woo_single_basic", $theplus_optionsget['check_elements']) && !empty($theplus_data['theplus_woo_swatches_switch'])){
...
function Tp_socialreview_Gettoken() {
	...
	if( wp_verify_nonce($delete_transient_nonce, 'SocialReview_nonce') ) {
		...
		if ( is_wp_error( $get_json ) ) {
			wp_send_json_error( array( 'messages' => 'something wrong' ) );
		}else{
...
function plus_acf_repeater_field_ajax(){
	...
	$data = [];	
	if(!empty($_GET['post_id']) && isset($_GET['post_id']) && absint($_GET['post_id'])){
	$acf_fields = get_field_objects($_GET['post_id']);
...
function theplus_pagination($pages = '', $range = 2)
	{  
		...
		if(1 != $pages)
		{
			...
			if ( $paged < $pages ) $paginate .= "<a class='paginate-next' href='".get_pagenum_link($paged + 1)."'>".esc_html__('NEXT','theplus')."<i class='fa fa-long-arrow-right' aria-hidden='true'></i></a>";
